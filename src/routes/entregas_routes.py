...
entregas_bp = Blueprint('entregas_bp', __name__)
...
def novo():
    if request.method == 'POST':
        ...
        dados_entregas.append(dados)
        
        flash('Registro adicionado com sucesso!', 'success')
        return redirect(url_for('entregas_bp.index'))
    
    return render_template('entregas/form.html',
                          registro={},
                          connection_status={'status': True, 'message': 'Conectado (simulado)'})
...
def editar(id):
    ...
    registro = next((item for item in dados_entregas if item['id'] == id), None)
    ...
    if request.method == 'POST':
        ...
        for i, item in enumerate(dados_entregas):
            if item['id'] == id:
                dados_entregas[i] = dados
                break
...
def excluir(id):
    ...
    global dados_entregas
    dados_entregas = [item for item in dados_entregas if item['id'] != id]
